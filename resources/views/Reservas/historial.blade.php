                    ...
                    <tbody>
                        @forelse($reservas as $reserva)
                            <tr class="border-t hover:bg-gray-100">
                                ...
                                <td class="px-4 py-2">
                                    {{ \Carbon\Carbon::parse($reserva->fecha_inicio)->format('Y-m-d') }}
                                </td>
                                <td class="px-4 py-2">
                                ...
                                <td class="px-4 py-2">
                                    ...
                                    @elseif($reserva->estado == 'confirmada')
                                        <span class="bg-green-100 text-green-800 px-2 py-1 rounded-full text-sm">Confirmada</span>
                                    @elseif($reserva->estado == 'cancelada')
                                        <span class="bg-red-100 text-red-800 px-2 py-1 rounded-full text-sm">Cancelada</span>
                                    @else
                                        <span class="bg-gray-100 text-gray-800 px-2 py-1 rounded-full text-sm">{{ $reserva->estado }}</span>
                                    @endif
                                </td>
                                <td class="px-4 py-2 text-center flex justify-center gap-2">
                                    <a href="{{ route('reservas.edit', $reserva->id_reserva) }}" 
                                       class="bg-yellow-500 hover:bg-yellow-400 text-white px-3 py-1 rounded-lg text-sm">
                                        Editar
                                    </a>
